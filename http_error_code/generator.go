...
func IsHttpCodeByTypeString(typeStr string) bool {
	return program.IsTypeNameString(typeStr, HttpErrorVarName) ||
		program.IsTypeNameString(typeStr, StatusErrorVarName)
}
	...
	for _, pkgInfo := range p.AllPackages {
		for ident, obj := range pkgInfo.Defs {
			if constObj, ok := obj.(*types.Const); ok {
				if IsHttpCodeByTypeString(p.TypeString(obj.Type())) {
					name := constObj.Name()
					if name == "_" {
						continue
					}
