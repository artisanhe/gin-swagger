...
import (
	...
	"git.chinawayltd.com/golib/gin-swagger/http_error_code"
	"git.chinawayltd.com/golib/gin-swagger/program"
)
	...
	if scope != nil {
		...
		if funType != nil {
			...
			for _, name := range scope.Names() {
				...
				if http_error_code.IsHttpCodeByTypeString(scanner.Program.TypeString(tpe)) {
				...
				case *types.Const:
					if len(scanner.HttpErr.HttpErrors) > 0 {
						...
						if !isHttpErrorMethodScope &&
							http_error_code.IsHttpCodeByTypeString(scanner.Program.TypeString(obj.Type())) {
...
func (scanner *Scanner) collectOperations() {
	var wg sync.WaitGroup
	wg.Add(len(scanner.Routes.Routers))

	for router := range scanner.Routes.Routers {
		go func(r *Router) {
			defer wg.Done()
			scanner.collectOperation(r.Method, r.GetPath(), r.GetArgs())
		}(router)
	}
